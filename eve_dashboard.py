...
function renderPlaneten(pl){
 ...
 if(piMsg&&(now-piMsg.ts)<7)html+=`<div class="sub pistat">${esc(piMsg.text)}</div>`;
  ...
  if(!isc)c.cols.forEach(col=>{
   let body=`<div class="picolhead"><b>${esc(col.planet)}</b> <span class="sub">${piTypeName(col.type,en)} · ${esc(col.system||'')} · ${en?'level':'Stufe'} ${col.upgrade||0} · ${col.pins||0} Pins</span>
     <span class="picolact">${col.isk?`<span class="isk">≈ ${fmtM(col.isk)} ISK ${en?'stored':'gelagert'}</span>`:''}
      <button class="pibtn" data-char="${esc(c.name)}" data-kind="info" data-id="${col.planet_id}" title="${en?'Open the planet in the running client (Show Info). The right character must be logged in.':'Planet im laufenden Client öffnen (Info-Fenster). Der Client dieses Chars muss eingeloggt sein.'}">🔎 ${en?'Show':'Zeigen'}</button>
      <button class="pibtn" data-char="${esc(c.name)}" data-kind="waypoint" data-id="${col.planet_id}" title="${en?'Set the planet as autopilot destination, warps you to it even within the system':'Planet als Autopilot-Ziel setzen, fliegt ihn auch innerhalb des Systems an'}">📍 ${en?'Fly there':'Anfliegen'}</button>
     </span></div>`;
...
// Ergebnis in piMsg, damit die Meldung die 2s-Ticks uebersteht. Der Erfolgstext
// ist bewusst kind-spezifisch, damit klar ist WAS passiert (nicht der PI-Editor).
let piMsg=null;
async function piOpen(char,kind,id){
 ...
 let txt;
 if(!r)txt=en?'client not reachable (open and online?)':'Client nicht erreichbar (offen und online?)';
 else if(!r.ok)txt=r.msg||'';
 else if(kind==='waypoint')txt=en?'set as destination, autopilot will warp you to the planet':'als Ziel gesetzt, Autopilot fliegt dich zum Planeten';
 else txt=en?'planet info opened (not the PI editor, ESI cannot do that)':'Planet-Info geöffnet (nicht der PI-Editor, das kann ESI nicht)';
 piMsg={ts:Date.now()/1000,text:char+': '+txt};
 renderPlaneten(lastPlaneten);
}
